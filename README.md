...
Once you have determined which redshift bin to use, please apply the trained multi-band `PSFGAN` (in that redshift bin) to remove AGN point sources. Then, in each of the five filters (or in a subset of filters that you interested), apply the trained `GaMorNet` (in that redshift bin) to predict morphology of the PSFGAN-recovered host galaxy in that particular filter.
