...
# This function is designed to be used in 'roouhsc.py'
def add_hsc_PSF(original, psf_flux, star_input, star_catalog, filter_string, num_star_per_psf,
                save_psf=False):
    """
    Args:
        original:          Original image.
        psf_flux:          Desired flux of the PSF that is being added to the
                           original image.
        star_input:        Folder containing selected stars
        star_catalog:      Dictionary containing the following HSC parameters
                           keywords: object_id, 'filter_string'+'_total_flux'
        filter_string:     Str(filter) (see roouhsc.py)
        num_star_per_psf:  Number of stars to create each model psf
        save_psf:          Whether to save the current model psf
    """

    star_path = '%s/*-%s.fits' % (star_input, filter_string)
    star_files = glob.glob(star_path)
    selected_files = random.sample(star_files, k=num_star_per_psf)

    selected_stars = []
    for i in range(len(selected_files)):
        file = selected_files[i]
        star_image_id = os.path.basename(file).replace('-' + filter_string + '.fits', '')
        star_obj_line = star_catalog.loc[star_catalog['object_id'] == int(star_image_id)]

        if star_obj_line.empty:
            print('Star not found in filter %s' % filter_string)
            continue

        star_flux = star_obj_line[filter_string+'_total_flux'].item()

        star = fits.getdata(file)
        star = star * (psf_flux / star_flux)
        selected_stars.append(star)

    model_psf = np.median(np.asarray(selected_stars), axis=0)

    #if not model_psf.shape == original.shape:
    #    print('Galaxy and star images have different shapes')
    #    if save_psf:
    #        return None, None
    #    else:
    #        return None

    if model_psf.shape == original.shape:
        reshaped_model_psf = model_psf
    else:
        if model_psf.shape[0] < original.shape[0]:
            # Assuming they are all square arrays
            model_psf_size = model_psf.shape[0]
            original_size = original.shape[0]
            width_zeros = int((original_size-1)/2 - (model_psf_size-1)/2)
            reshaped_model_psf = np.pad(model_psf, ((width_zeros,width_zeros),(width_zeros,width_zeros)), 'constant')


    conditional_input = original + reshaped_model_psf

    if save_psf:
        return conditional_input, reshaped_model_psf
    else:
        return conditional_input


# We define a function: radio_profile(fits image) = radio profile
